...
time2u <- system.time(out2u <- ci_bound_wn_i(7, 38, sem_out = fit, which = "ubound", opts = opts0, f_constr = fn_constr0, verbose = TRUE, ciperc = ciperc))

test_that("Check against precomputed answers", {
    expect_equal(out1l$bound, 0.6624193, tolerance = 1e-5)
    expect_equal(out2u$bound, 0.4651715, tolerance = 1e-5)
  })
