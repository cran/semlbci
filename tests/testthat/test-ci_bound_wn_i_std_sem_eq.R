...
time1l <- system.time(out1l <- ci_bound_wn_i(5, 13, sem_out = fit, which = "lbound", opts = opts0, f_constr = fn_constr0, verbose = TRUE, ciperc = ciperc, standardized = TRUE, wald_ci_start = FALSE, std_method = "internal"))
...
test_that("Check against precomputed answers", {
    expect_equal(out1l$bound, 0.638426, tolerance = 1e-5)
    expect_equal(out2u$bound, 0.626015, tolerance = 1e-5)
  })
