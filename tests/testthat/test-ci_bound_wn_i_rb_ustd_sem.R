...
time2u <- system.time(out2u <- ci_bound_wn_i(7, 13, sem_out = fit, f_constr = fn_constr0, which = "ubound", opts = opts0, verbose = TRUE, ciperc = ciperc, sf = sf2$c_r, sf2 = sf1$c_rb))

test_that("Check against precomputed answers", {
    expect_equal(out1l$bound, 1.047957, tolerance = 1e-5)
    expect_equal(out2u$bound, 0.5121379, tolerance = 1e-5)
  })
